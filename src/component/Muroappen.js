...
import { showPostFunt } from './postNew.js';
...
export const muro = () => {
  ...
  const newPost = document.createElement('input');// aquiiiiiiiiiii
  ...
  // newPost.addEventListener('', (e) => {
  //   const newPostValue = e.target.value;
  //   console.log(newPostValue);
  // });
  const inputRe = muroDiv.querySelector('.newPost'); // elemento input
  const currentUser = auth.currentUser;

  buttonsavepost.addEventListener('click', () => {
    savePost(currentUser.uid, currentUser.displayName, inputRe);
  });

  showPostFunt(muroDiv);
  return muroDiv;
};

// 3.FUNCION PARA CREAR CALABERA POST DE FIREBASE-----------------

const createPost = function createPost(userName, postDescription, idpost) {
...
const savePost = (uId, userName, inputRe) => {
  savebdPost(uId, userName, inputRe.value);
};

 // 5. mostrar Post en muro------------------------------
const mostrarPosts = function (querySnapshot) {
  querySnapshot.forEach((doc) => {
    const bdmuro = doc.data();
   let x= createPost(bdmuro.postDescription, bdmuro.userName, doc.id);
   console.log(x);
  });
};
